...
| [x] | W | Panel mask UNOSAT | Gambar panel | 4.2 | Menjelaskan perbedaan `label_flood_binary`, `label_valid_mask`, dan `water_river_mask`. |
...
| [x] | W | Jumlah tile train/val/test per fold | Tabel | 4.3 | Memastikan ukuran data tiap fold transparan. |
...
| [x] | W | Panel mask UNOSAT | Gambar panel | `label_flood_binary`, `label_valid_mask`, `water_river_mask` pada wilayah representatif. | `dataset/labels_unosat_rasterized/<region>/` |
...
| [x] | O | Contoh tile positif dan tile background | Gambar panel | Tile positif dan tile background-only, minimal VV/HSV/HAND + label. | `dataset/tiles/.../*.npz` |
| [x] | N | Penjelasan fungsi mask | Narasi | Tegaskan `FloodExtent` menjadi label banjir; `AnalysisExtent` menjadi valid mask; `WaterExtent`/`River` auxiliary, bukan label utama. | Hasil rasterisasi UNOSAT |
...
| [x] | W | Jumlah tile train/val/test per fold | Tabel | Fold, jumlah tile train, jumlah tile validation, jumlah tile test, total tile. | `dataset/tiles/`, metadata fold |
| [x] | D | Diagram skema spatial split | Diagram | Aceh_Utara sebagai final test, 10 wilayah lain sebagai 5-fold CV. | Daftar wilayah/fold |
| [x] | N | Alasan menghindari random split tile | Narasi | Jelaskan spatial leakage dan kenapa spatial split lebih realistis. | Desain eksperimen |
| [x] | N | Kesimpulan validasi eksperimental | Narasi | Tegaskan model diuji pada wilayah yang tidak pernah dilihat. | Tabel fold + split |
...
| [x] | O | Diagram U-Net aktual | Diagram | Input 7ch → encoder → bottleneck → decoder → logit 1ch. | Implementasi U-Net |
| [x] | SD | Diagram ProCANet aktual | Diagram | Encoder 1 7ch + Encoder 2 2ch → PCAB skip/bottleneck → decoder → logit 1ch. | Implementasi ProCANet |
| [x] | N | Penjelasan output logit | Narasi | Output model adalah logit 1-channel; mask biner diperoleh dari sigmoid + threshold 0,5 saat inferensi. | Implementasi + evaluasi |
...
| [x] | N | Interpretasi weight decay | Narasi | Jelaskan efek regularisasi terhadap overfitting/underfitting. | Tabel grid search |
| [x] | N | Keterbatasan ruang grid search | Narasi | Tegaskan hanya enam kombinasi lr/wd yang diuji. | Desain tuning |
...
| [x] | N | Penjelasan checkpoint terbaik | Narasi | Checkpoint terbaik dipilih berdasarkan validation IoU. | Training metrics |
| [x] | N | Penjelasan loss | Narasi | Loss adalah masked BCE + Dice Loss; hanya piksel valid yang dihitung. | Training implementation |
| [x] | N | Interpretasi overfitting/stabilitas | Narasi | Bahas jarak train-val, stagnasi IoU, penurunan learning rate, dan early stopping. | Grafik training |
...
| [x] | O | Bar chart FP/FN | Grafik batang | FP dan FN U-Net vs ProCANet. | Confusion matrix |
| [x] | N | Interpretasi IoU dan Dice | Narasi | Jelaskan model mana yang tumpang tindihnya lebih baik terhadap UNOSAT. | Tabel metrik |
| [x] | N | Interpretasi FP/FN | Narasi | Jelaskan model agresif vs konservatif. | Confusion matrix |
| [x] | N | Posisi akurasi | Narasi | Akurasi hanya metrik pelengkap karena kelas non-banjir dominan. | Tabel metrik |
...
| [x] | O | Tabel aspek visual model | Tabel | Aspek visual, U-Net, ProCANet: cakupan banjir, FP, FN, batas prediksi, konsistensi label. | Panel prediksi + error map |
| [x] | N | Interpretasi area benar | Narasi | Jelaskan bagian yang berhasil dideteksi oleh kedua model. | Panel visual |
...
| [x] | N | Pengait visual-kuantitatif | Narasi | Tegaskan visual harus dibaca bersama metrik, bukan sebagai bukti tunggal. | 4.5 + 4.6 |
...
| [x] | W | Jumlah tile train/val/test per fold | 4.3 |
...
| [x] | W | Panel mask UNOSAT | Gambar panel | 4.2 |
| [x] | O | Contoh tile positif dan background | Gambar panel | 4.2 |
| [x] | D | Diagram spatial split | Diagram | 4.3 |
| [x] | O | Diagram U-Net aktual | Diagram | 4.4.1 |
| [x] | SD | Diagram ProCANet aktual | Diagram | 4.4.1 |
...
| [x] | O | Bar chart FP/FN | Grafik batang | 4.5 atau 4.7 |
| [x] | W | Panel prediksi U-Net vs ProCANet | Gambar panel | 4.6 |
| [x] | SD | Error map TP/FP/FN/TN | Gambar | 4.6 |
| [x] | O | Contoh tile HSV=0 | Gambar panel | 4.8 |
| [x] | O | Studi kasus topografi/radar shadow | Gambar panel | 4.8 |
| [x] | O | Studi kasus badan air permanen | Gambar panel | 4.8 |
...
| [x] | 4.2 | Perbedaan fungsi `label_flood_binary`, `label_valid_mask`, dan `water_river_mask`. |
...
| [x] | 4.4.2 | Interpretasi efek learning rate dan weight decay. |
...
| [x] | Warna/legend error map jelas membedakan TP, FP, FN, dan TN. |

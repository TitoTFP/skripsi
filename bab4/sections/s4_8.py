...
SUPPLEMENTARY = (
    ArtifactSpec(
        ...
        "4_8_oof_extreme_condition_metrics_by_region.csv",
...
def generate_4_8(config):
    ...
    if not all(path.exists() for path in required):
        ...
        artifacts.extend(
            [
                missing_result(config, _spec("Tabel 4.19"), source=OOF_SOURCE, note=note),
                missing_result(config, _spec("Tabel 4.20"), source=OOF_SOURCE, note=note),
                missing_result(config, _spec("Tambahan 4.8"), source=OOF_SOURCE, note=note),
                *[missing_result(config, _spec(artifact_id), source=OOF_SOURCE, note=note) for artifact_id in ("Gambar 4.18", "Gambar 4.19", "Gambar 4.20")],
                _narrative_4_8(config, [], available=False),
            ]
        )
    ...
    artifacts.extend(
        [
            write_table(config, _spec("Tabel 4.19"), _selected_tile_rows(selected_tiles), source=OOF_SOURCE),
            write_table(config, _spec("Tabel 4.20"), _micro_metric_rows(micro_rows), source=OOF_SOURCE),
            write_table(config, _spec("Tambahan 4.8"), _per_region_metric_rows(per_region_rows), source=OOF_SOURCE),
            *[
                _figure_extreme_oof(config, artifact_id, selected_tiles, provenance)
                for artifact_id in ("Gambar 4.18", "Gambar 4.19", "Gambar 4.20")
            ],
            _narrative_4_8(config, _micro_metric_rows(micro_rows), available=True),
        ]
    )
    return section_result("4.8", artifacts)
...
def _remove_obsolete_outputs(config) -> None:
    obsolete = (
        "4_8_unet_vs_procanet_effectiveness_summary.csv",
        "4_8_literature_context_comparison.csv",
        "4_9_bab4_findings_summary.csv",
        "4_8_unet_procanet_effectiveness_discussion.md",
        ...
        "4_7_difficult_data_case_studies.csv",
        "4_7_extreme_tile_selection.csv",
        "4_7_oof_extreme_condition_metrics.csv",
        "4_7_oof_extreme_condition_metrics_by_region.csv",
        "4_7_difficult_data_case_studies.png",
        "4_7_hsv_zero_tile_panel.png",
        "4_7_topography_radar_shadow_case.png",
        "4_7_permanent_water_case.png",
        "4_7_extreme_condition_oof_interpretation.md",
    )
...
def _difficult_data_rows(config) -> list[dict[str, object]]:
    ...
    return sorted(rows, key=lambda row: (to_float(row["s2_valid_pct_of_valid"]), str(row["wilayah"])))
...
def _figure_difficult_cases(config, rows: list[dict[str, object]]):
    spec = _spec("Gambar 4.17")
    setup_style()
    fig, ax = plt.subplots(figsize=(8.4, 5.0))
    s2 = np.asarray([to_float(row["s2_valid_pct_of_valid"]) for row in rows])
    flood = np.asarray([to_float(row["flood_pct_of_valid"]) for row in rows])
    water = np.asarray([to_float(row["water_river_pct_of_valid"]) for row in rows])
...
def _figure_extreme_oof(config, artifact_id: str, selected_tiles: list[dict[str, str]], provenance: dict[str, object]):
    condition = {
        "Gambar 4.18": "s2_empty",
        "Gambar 4.19": "topography_radar_shadow",
        "Gambar 4.20": "permanent_water",
    }[artifact_id]
    ...
    with np.load(path, allow_pickle=False) as tile:
        ...
        try:
            ...
            raise ValueError(f"metadata tile tidak valid: {path}") from exc
...
def _tile_condition(condition: str, x: np.ndarray, effective: np.ndarray, label: np.ndarray, water: np.ndarray, s2: np.ndarray, thresholds: dict[str, object]) -> np.ndarray:
    if condition == "s2_empty":
        try:
            s2_valid_ratio = float(s2[effective].mean()) if effective.any() else 1.0
        except (TypeError, ValueError) as exc:
            raise ValueError("mask Sentinel-2 tidak valid") from exc
        return effective if s2_valid_ratio <= 0.01 else np.zeros_like(effective)
    if condition == "topography_radar_shadow":
        return effective & (x[5] * 45.0 > 20.0) & (x[6] * 50.0 > 40.0) & ((x[0] <= to_float(thresholds["p20_vv_norm"])) | (x[1] <= to_float(thresholds["p20_vh_norm"])))
...
def _narrative_4_8(config, rows: list[dict[str, object]], *, available: bool):
    ...
    if not available:
        ...
        text = f"""
        Tabel 4.20 menghitung performa dengan prediksi out-of-fold pada sepuluh wilayah cross-validation.
